"""Specialized Neural Reactivation Specialist (P(will_buy | inactive)).

Trained strictly on inactive users (was_active == False) with binary cross entropy on will_buy.
"""
...
class NeuralReactivationSpecialist(nn.Module):
    def __init__(self, encoder: nn.Module, d_model: int = 128, dropout: float = 0.10):
        super().__init__()
        self.encoder = encoder
        self.d_model = d_model

        # Dedicated Reactivation Head
        self.reactivation_head = nn.Sequential(
    ...
    def forward(self, *inputs) -> torch.Tensor:
        ...
        logit_react = self.reactivation_head(emb).squeeze(-1)
        return logit_react
