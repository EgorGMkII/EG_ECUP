...
def fit_meta(bank: dict[str, np.ndarray], schema: PredictionSchema, *, root_seed: int, prediction_bank_sha256: str, commit_sha: str, config_sha256: str) -> dict[str, Any]:
    ...
    alpha = 1.1
    canonical = np.r_[np.full(r, 1.0 / r), np.full(c, 1.0 / c), np.ones(a), 0.0]
    rng = np.random.default_rng(root_seed)
    starts = [canonical] + [np.r_[rng.dirichlet(np.ones(r)), rng.dirichlet(np.ones(c)), rng.random(a), 0.0] for _ in range(8)]
    constraints = [
        {"type": "eq", "fun": lambda x: x[:r].sum() - 1.0},
        {"type": "eq", "fun": lambda x: x[r:r+c].sum() - 1.0},
    ]
    bounds = [(0.0, 1.0)] * (r + c) + [(0.0, None)] * a + [(None, None)]
    ...
    package: dict[str, Any] = {
        ...
        "feature_order": {"react": list(schema.react_columns), "churn": list(schema.churn_columns), "amount": list(schema.amount_columns)},
        "alpha": alpha,
...
def apply_meta_components(package: dict[str, Any], bank: dict[str, np.ndarray], schema: PredictionSchema) -> dict[str, np.ndarray]:
    expected = {"react": list(schema.react_columns), "churn": list(schema.churn_columns), "amount": list(schema.amount_columns)}
    if package.get("meta_schema_version") != META_SCHEMA_VERSION or package.get("feature_order") != expected:
    ...
    return {"p_react": p_react, "p_churn": p_churn, "p_buy": p_buy, "conditional_z": conditional, "prediction_z": _predict(bank, params, np.asarray(package["amount_mean"]), np.asarray(package["amount_scale"]), schema, float(package["alpha"]))}
