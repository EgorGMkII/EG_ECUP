"""Canonical Step-Level Controlled Learning Rate Experiments for Event-Time Transformer (ETT1).

Implements:
- Exact canonical feature extraction (12 content + 12 time features, tau=30d)
- 20,000 fixed validation monitoring subset
- Evaluation every 250 steps (monitoring 20k) and every 1,000 steps (full 100k)
- Linear Warmup (500 steps) + Cosine Decay to 0.10 * max_lr
- Early stopping after 4,000 steps with 2,000-step patience
- Runs OPT_LR0 (3e-4), OPT_LR1 (1e-4), OPT_LR2 (5e-5)
- Full transition metrics, checkpoint selection strictly by full validation
"""
...
def compute_event_sequences_for_anchor(
    ...
    time_feats = np.zeros((n_users, max_events, 12), dtype=np.float32)
    ranks_arr = np.zeros((n_users, max_events), dtype=np.int64)
    padding_mask = np.ones((n_users, max_events), dtype=bool)
    ...
    return content_feats, time_feats, ranks_arr, padding_mask, is_empty_arr
...
class EventSequenceDataset(Dataset):
    def __init__(self, content, time_feat, ranks, mask, empty, z_true, was_active, y_rub):
        ...
        self.was_active = torch.tensor(was_active, dtype=torch.float32)
...
# -----------------------------------------------------------------------------
# 2. Canonical ETT1 Model Architecture
# -----------------------------------------------------------------------------
...
class EventTimeTransformer(nn.Module):
    def __init__(
        ...
        # 2. Transformer Encoder (Pre-LN, GELU)
    ...
    def forward(
        ...
        last_nonempty_token = h[:, -1, :]  # Right-aligned: last token is pos -1
...
def compute_canonical_loss(
    ...
    # 3. Conditional MSE on positive spenders
...
def run_single_lr_experiment(
    ...
    max_steps: int = 12000,
    min_steps: int = 4000,
    eval_monitoring_every: int = 250,
    eval_full_every: int = 1000,
    patience_steps: int = 2000,
    seed: int = 42,
) -> Dict:
    print(f"\n{'='*80}\n[*] STARTING RUN: {run_name} (max_lr={max_lr:.1e})\n{'='*80}")
    ...
    for global_step in range(1, max_steps + 1):
        ...
        # Step validation on Monitoring Subset
        if global_step % eval_monitoring_every == 0:
            mon_metrics, _, _ = evaluate_dataset(model, monitoring_dataset, batch_size=2048, device=device)
            ...
            if is_new_best:
                ...
                steps_without_improvement += eval_monitoring_every
            ...
            log_entry = {
                ...
                "train_loss_fact": loss_dict["loss_fact"],
                "train_loss_dir": loss_dict["loss_dir"],
                "mon_rmsle_fact": mon_fact,
                "mon_rmsle_dir": mon_dir,
                "is_best_mon": is_new_best,
            }

            if global_step % eval_full_every == 0:
                full_metrics, _, _ = evaluate_dataset(model, val_dataset, batch_size=2048, device=device)
                log_entry.update({
                ...
                print(
                    ...
                    f"Train Loss: {loss_dict['loss_total']:.4f} | "
    ...
    print(f"[*] {run_name} finished training in {elapsed/60:.1f} min. Re-evaluating candidate checkpoints...")
    ...
    for cand in candidate_checkpoints:
        ...
        metrics, preds_fact, preds_dir = evaluate_dataset(model, val_dataset, batch_size=2048, device=device)
    ...
    print(f"\n[+] {run_name} WINNER CHECKPOINT: Step {best_final_checkpoint['step']} with FULL Fact RMSLE = {best_full_rmsle:.5f}\n")

    run_dir = out_dir / run_name
    run_dir.mkdir(parents=True, exist_ok=True)
    torch.save(best_final_checkpoint["state_dict"], run_dir / "best_model.pt")

    df_preds = pl.DataFrame({
        "pred_factorized_z": best_full_preds_fact,
        "pred_direct_z": best_full_preds_dir,
    })
    df_preds.write_parquet(run_dir / "validation_predictions.parquet")

    return {
        "run_name": run_name,
        "max_lr": max_lr,
        "best_step": best_final_checkpoint["step"],
        "examples_seen": best_final_checkpoint["step"] * batch_size,
...
def main():
    print("=" * 80)
    print("CANONICAL ETT1 STEP-LEVEL OPTIMIZATION SUITE (OPT_LR0, OPT_LR1, OPT_LR2)")
    print("=" * 80)
    ...
    if not users_path.exists():
        ...
        user_ids = snap_val["user_id"].to_numpy()[:100000]
    else:
    ...
    print(f"[*] Reading raw events from {train_path}...")
    df_raw = pl.read_parquet(train_path)

    # 2. Extract Validation Anchor (2026-01-14)
    print("\n[*] Extracting Validation Sequences (2026-01-14)...")
    val_c, val_t, val_r, val_m, val_emp = compute_event_sequences_for_anchor(
        df_raw, VAL_ANCHOR, user_ids, max_events=128, tau_days=30.0
    )
    snap_val = pl.read_parquet(snapshots_dir / f"snapshot_{VAL_ANCHOR}.parquet")
    val_target = snap_val["target"].to_numpy()
    val_target_z = np.log1p(np.maximum(0.0, val_target))
    ...
    # 3. Create Fixed 20k Monitoring Subset (stratified by 4 transition states)
    monitoring_path = out_dir / "monitoring_users_20k.parquet"
    if monitoring_path.exists():
        ...
        pl.DataFrame({"val_index": monitoring_indices, "user_id": user_ids[monitoring_indices]}).write_parquet(monitoring_path)
        print(f"[+] Created and saved {len(monitoring_indices)} monitoring indices to {monitoring_path}")

    # 4. Extract 11 Train Anchors Sequences (1.1M pairs)
    print("\n[*] Extracting 11 Training Anchors Sequences...")
    train_c_list, train_t_list, train_r_list, train_m_list, train_emp_list = [], [], [], [], []
    train_z_list, train_act_list, train_rub_list = [], [], []

    for anchor in ANCHORS_V1:
        print(f"  -> Loading anchor {anchor}...")
        c, t, r, m, emp = compute_event_sequences_for_anchor(
            df_raw, anchor, user_ids, max_events=128, tau_days=30.0
        )
        snap = pl.read_parquet(snapshots_dir / f"snapshot_{anchor}.parquet")
        target = snap["target"].to_numpy()
        target_z = np.log1p(np.maximum(0.0, target))
        was_act = (snap["lifetime_gmv"].to_numpy() > 0).astype(int)

        train_c_list.append(c)
        train_t_list.append(t)
        train_r_list.append(r)
        train_m_list.append(m)
        train_emp_list.append(emp)
        train_z_list.append(target_z)
        train_act_list.append(was_act)
        train_rub_list.append(target)

    all_train_c = np.concatenate(train_c_list, axis=0)
    all_train_t = np.concatenate(train_t_list, axis=0)
    all_train_r = np.concatenate(train_r_list, axis=0)
    all_train_m = np.concatenate(train_m_list, axis=0)
    all_train_emp = np.concatenate(train_emp_list, axis=0)
    all_train_z = np.concatenate(train_z_list, axis=0)
    all_train_act = np.concatenate(train_act_list, axis=0)
    all_train_rub = np.concatenate(train_rub_list, axis=0)
    ...
    print(f"[+] Complete Training Dataset Ready: {len(train_dataset)} sequences (1.1M pairs).")

    # 5. Run Controlled LR Experiments
    lr_configs = [
        {"run_name": "OPT_LR0", "max_lr": 3e-4},
        {"run_name": "OPT_LR1", "max_lr": 1e-4},
        {"run_name": "OPT_LR2", "max_lr": 5e-5},
    ]
    ...
    for cfg in lr_configs:
        res = run_single_lr_experiment(
            ...
            max_steps=12000,
            min_steps=4000,
            eval_monitoring_every=250,
            eval_full_every=1000,
            patience_steps=2000,
            seed=42,
        )
        all_summaries.append({
            ...
            "best_full_fact_rmsle": res["best_full_fact_rmsle"],
            "best_full_dir_rmsle": res["best_full_dir_rmsle"],
            "mse_0_0": res["mse_0_0"],
            "mse_0_1": res["mse_0_1"],
            "mse_1_0": res["mse_1_0"],
            "mse_1_1": res["mse_1_1"],
        })
        all_step_logs.extend(res["step_logs"])

    # 6. Save Summaries & Curves
    df_summary = pl.DataFrame(all_summaries)
    df_summary.write_csv(out_dir / "lr_experiment_summary.csv")
    print(f"\n[+] Saved {out_dir / 'lr_experiment_summary.csv'}")

    df_curves = pl.DataFrame(all_step_logs)
    df_curves.write_csv(out_dir / "step_level_training_curves.csv")
    print(f"[+] Saved {out_dir / 'step_level_training_curves.csv'}")

    print("\n" + "=" * 80)
    print("LR EXPERIMENTS FINAL SUMMARY:")
    print("=" * 80)
    print(df_summary)
