"""Scale-Up Experiment for Event-Time Transformer on ALL 250,000 Users (OPT_ALL250K).

Specifications:
- 11 training anchors across all 250,000 users = 2,750,000 training pairs
- max_events = 128 event-days
- micro_batch_size = 128, grad_accum_steps = 4 (effective batch_size = 512)
- max_lr = 3e-4 (warmup = 500 steps, cosine decay to 10%)
- Step validation: monitoring on 20k subset every 250 steps, full 100k val every 1000 steps
- Early stopping patience: 2000 steps after min 3000 steps, max 10000 steps
"""
...
def main():
    print("=" * 80)
    print("SCALE-UP EXPERIMENT ON ALL 250,000 USERS: OPT_ALL250K")
    print("=" * 80)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"[*] Execution device: {device}")

    out_dir = Path("artifacts/ett_optimization/OPT_ALL250K")
    ...
    # Get all 250k users from full validation snapshot
    snap_val = pl.read_parquet(snapshots_dir / f"snapshot_{VAL_ANCHOR}.parquet")
    user_ids_250k = snap_val["user_id"].to_numpy()
    n_users = len(user_ids_250k)
    print(f"[+] Loaded {n_users} full cohort users from {snapshots_dir / f'snapshot_{VAL_ANCHOR}.parquet'}.")

    df_raw = pl.scan_parquet(train_path)

    # 1. Validation Buffers (100k canonical users for strict benchmark comparability)
    ...
    users_100k_df = pl.read_parquet(users_100k_path)
    val_user_ids = users_100k_df["user_id"].to_numpy()
    n_val_users = len(val_user_ids)

    print(f"\n[*] Preallocating and Extracting Benchmark Validation Sequences (100k users, {VAL_ANCHOR})...")
    val_c = np.zeros((n_val_users, 128, 12), dtype=np.float32)
    val_t = np.zeros((n_val_users, 128, 12), dtype=np.float32)
    val_r = np.zeros((n_val_users, 128), dtype=np.int16)
    val_m = np.ones((n_val_users, 128), dtype=bool)
    val_emp = np.zeros(n_val_users, dtype=bool)

    extract_features_into_buffers(
        df_raw, VAL_ANCHOR, val_user_ids, val_c, val_t, val_r, val_m, val_emp,
        offset=0, max_events=128, tau_days=30.0
    )

    # Filter snapshot to 100k users
    snap_val_100k = snap_val.filter(pl.col("user_id").is_in(set(val_user_ids))).sort("user_id")
    # Align order with val_user_ids
    user_to_idx = {u: i for i, u in enumerate(snap_val_100k["user_id"].to_list())}
    order = [user_to_idx[u] for u in val_user_ids]
    val_target = snap_val_100k["target"].to_numpy()[order].astype(np.float32)
    val_target_z = np.log1p(np.maximum(0.0, val_target)).astype(np.float32)
    val_was_act = (snap_val_100k["lifetime_gmv"].to_numpy()[order] > 0).astype(np.float32)

    val_dataset = ZeroCopyDataset(
        val_c, val_t, val_r, val_m, val_emp, val_target_z, val_was_act, val_target
    )
    print(f"[+] Validation benchmark dataset ready: {len(val_dataset)} samples.")
    ...
    if monitoring_path.exists():
        ...
        pl.DataFrame({"val_index": monitoring_indices, "user_id": val_user_ids[monitoring_indices]}).write_parquet(out_mon)
    ...
    # 3. Preallocate and Fill ALL 250k Users across 11 Training Anchors (2.75M pairs) In-Place
    n_train_samples = len(ANCHORS_V1) * n_users
    print(f"\n[*] Preallocating Contiguous Training Buffers: {n_train_samples} samples (ALL 250k users, 128 tokens)...")
    all_train_c = np.zeros((n_train_samples, 128, 12), dtype=np.float32)
    all_train_t = np.zeros((n_train_samples, 128, 12), dtype=np.float32)
    all_train_r = np.zeros((n_train_samples, 128), dtype=np.int16)
    all_train_m = np.ones((n_train_samples, 128), dtype=bool)
    ...
    for k, anchor in enumerate(ANCHORS_V1):
        offset = k * n_users
        print(f"  -> Extracting anchor {anchor} ({k+1}/{len(ANCHORS_V1)}) for all {n_users} users...")
        extract_features_into_buffers(
            df_raw, anchor, user_ids_250k,
            all_train_c, all_train_t, all_train_r, all_train_m, all_train_emp,
            offset=offset, max_events=128, tau_days=30.0
        )
        snap = pl.read_parquet(snapshots_dir / f"snapshot_{anchor}.parquet")
        # Align snapshot with user_ids_250k
        u_map = {u: i for i, u in enumerate(snap["user_id"].to_list())}
        snap_order = [u_map[u] for u in user_ids_250k]
        target = snap["target"].to_numpy()[snap_order].astype(np.float32)
        all_train_rub[offset : offset + n_users] = target
        all_train_z[offset : offset + n_users] = np.log1p(np.maximum(0.0, target))
        all_train_act[offset : offset + n_users] = (snap["lifetime_gmv"].to_numpy()[snap_order] > 0).astype(np.float32)

    train_dataset = ZeroCopyDataset(
        all_train_c, all_train_t, all_train_r, all_train_m, all_train_emp,
        all_train_z, all_train_act, all_train_rub
    )
    print(f"[+] Full-Scale Training Dataset Ready: {len(train_dataset)} sequences (2.75M pairs).")

    # 4. Train Model with Effective Batch Size = 512
    ...
    grad_accum_steps = 4  # Effective batch size = 512
    ...
    print(f"\n[*] Training OPT_ALL250K (effective bs=512, max_lr={max_lr:.1e}, max_events=128)...")
    ...
    print(f"[*] OPT_ALL250K finished training in {elapsed/60:.1f} min. Re-evaluating candidate checkpoints...")
    ...
    print(f"\n[+] OPT_ALL250K WINNER CHECKPOINT: Step {best_final_checkpoint['step']} with FULL Fact RMSLE = {best_full_rmsle:.5f}\n")
    ...
    summary = [{
        "run_name": "OPT_ALL250K",
        "n_users": n_users,
        "n_train_samples": n_train_samples,
        "max_events": 128,
        "max_lr": max_lr,
        "best_step": best_final_checkpoint["step"],
        "examples_seen": best_final_checkpoint["step"] * 512,
    ...
    pl.DataFrame(summary).write_csv(out_dir / "all250k_summary.csv")
    pl.DataFrame(step_logs).write_csv(out_dir / "all250k_training_curves.csv")
    print(f"[+] Saved summary to {out_dir / 'all250k_summary.csv'}")
