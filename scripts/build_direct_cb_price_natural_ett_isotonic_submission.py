"""Full-history submission builder: 0.70 CatBoost Price Tier Natural + 0.30 ETT Direct with Multi-Season Isotonic Calibration."""
...
def main() -> None:
    ...
    ap.add_argument("--output-root", type=Path, default=Path("artifacts/direct_cb_price_natural_ett_isotonic_submission"))
    ap.add_argument("--calibrator-json", type=Path, default=Path("src/direct_temporal_cv_v1/calibrators/isotonic_blend_f1_f3.json"))
    ...
    print("[*] Training CatBoost Price Tier Natural Specialist (p_power=1.0)...", flush=True)
    ...
    # Blend: 0.70 CatBoost + 0.30 ETT
    print("[*] Blending raw predictions (0.70 CB + 0.30 ETT)...", flush=True)
    z_blend_raw = 0.70 * cb_res.prediction_z + 0.30 * ett_res.prediction_z

    # Apply 3-Season Isotonic Calibration
    print(f"[*] Applying 3-Season Isotonic Calibration from {args.calibrator_json}...", flush=True)
    calib_data = json.loads(args.calibrator_json.read_text(encoding="utf-8"))
    x_knots = np.array(calib_data["x_thresholds"])
    y_knots = np.array(calib_data["y_thresholds"])
    
    # Piecewise linear/constant interpolation matching sklearn IsotonicRegression
    calib_fn = interp1d(x_knots, y_knots, kind="linear", bounds_error=False, fill_value=(y_knots[0], y_knots[-1]))
    z_blend_cal = calib_fn(z_blend_raw)
    z_blend_cal = np.maximum(z_blend_cal, 0.0)

    pred = np.maximum(np.expm1(z_blend_cal), 0.0)
    ...
    report = {
        "experiment_id": "direct_cb_price_natural_ett_isotonic_submission_v1",
        ...
        "calibration": "isotonic_f1_f2_f3_multi_season",
        "weights": {
            "catboost_cohort_specialist_price_natural": 0.70,
            "ett_direct": 0.30,
        },
        "models": {
            "catboost_cohort_specialist_price_natural": cb_res.training_report,
